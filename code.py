...
os.chdir('/nas/vc_project/Prometrix/travelers/shp')

if not os.path.exists('output'):
	os.makedirs('output')
outpath='/nas/vc_project/Prometrix/travelers/shp/output'
...
for county in  CAcountylist:
	inshp="CA/%s/%sloc.shp"%(county,county)
	outshp="CA/%s/%slocrepro.shp"%(county,county)
	...
	with rasterio.open("GRID/CA/%s/%sfrshiag/w001001.adf"%(county,county),"r") as src:
	...
	outputf = "output/%s.nc"%(county)
	args = ['shape2grid.py',outshp,outputf,'AREASCORE','--ll', str(ymin),str(xmin),	 '--ur', str(ymax),str(xmax), '-s',str(dst_height), str(dst_width),'--attr','AREASCORE','CA']	
