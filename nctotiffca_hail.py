...
for st in state:
	...
	for county in ls:
		print (county)
		try:
			infile=net.Dataset(r"D:\twu\fireline\travelers\test\peo_WA_WFHS.nc")
			lats=infile.variables['latitude'][:]
			lons=infile.variables['longitude'][:]
			ymin=lats.min()
			ymax=lats.max()
			xmin=lons.min()
			xmax=lons.max()
			...
			value=infile.variables['AREASCORE'][:]
			...
			outf='D:/twu/fireline/travelers/tiffs/peop_WA_WFHS.tiff'


			with rasterio.drivers():
				trans=(0.0003,0.0,xmin,0.0, -0.0003, ymax)
				with rasterio.open(outf,"w",driver='GTiff',dtype="uint8", nodata=255,compress="lzw",height=srsheight,count=1, width=srswidth,transform=trans,crs="EPSG:4326") as dest:
					dest.write(value,1)
